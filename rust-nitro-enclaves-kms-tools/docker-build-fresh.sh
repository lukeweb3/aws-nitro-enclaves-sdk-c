...
docker build --no-cache -f rust-nitro-enclaves-kms-tools/Dockerfile.al2 --target kmstool-enclave-rust -t kmstool-enclave-rust:latest .
docker build --no-cache -f rust-nitro-enclaves-kms-tools/Dockerfile.al2 --target kmstool-instance-rust -t kmstool-instance-rust:latest .
docker build --no-cache -f rust-nitro-enclaves-kms-tools/Dockerfile.al2 --target kmstool-enclave-cli-rust -t kmstool-enclave-cli-rust:latest .
docker build --no-cache -f rust-nitro-enclaves-kms-tools/Dockerfile.al2 --target development -t nitro-enclaves-rust-dev:latest .
